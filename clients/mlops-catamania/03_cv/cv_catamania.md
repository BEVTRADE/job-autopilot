% HAKIM AREZKI, PhD
% ARCHITECTE IA ET MLOPS — DU CADRAGE AU PROTOTYPE, DU PROTOTYPE À LA PRODUCTION
...
Conduit le cycle complet : étude de faisabilité, architecture cible, prototype court de quatre à six semaines tranché sur des critères mesurés, puis industrialisation. Couvre aussi bien le Machine Learning prédictif que le RAG et l'IA générative, et choisit entre les trois selon le problème plutôt que selon la mode. Référent technique auprès des équipes sur la durée.
...
- Étude de faisabilité : disponibilité et qualité des données, niveau de fiabilité atteignable, coût de mise en œuvre et coût récurrent
- Prototypage court, quatre à six semaines, avec critères d'arrêt définis avant de commencer
- Machine Learning prédictif, classification, détection d'anomalies, vision par ordinateur, séries temporelles
- Pipelines d'entraînement, d'évaluation et de déploiement, MLflow, registre de modèles, versionnement des données et du code
- Intégration et déploiement continus pour les modèles, les images et les prompts, GitLab CI et GitHub Actions
- Observabilité ML : dérive des données et des prédictions, dégradation de performance, alertes et réentraînement
- Observabilité LLM : traces, coût, latence p50 et p95, taux d'échec, qualité et factualité
- Déploiement sur Azure et environnements cloud, conteneurisation Docker, orchestration Kubernetes
- Rôle de référent technique IA : standards, décisions structurantes formalisées en ADR, revue des choix d'architecture des équipes

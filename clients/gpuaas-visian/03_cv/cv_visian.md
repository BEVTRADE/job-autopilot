% HAKIM AREZKI, PhD
% ARCHITECTE ET LEAD TECHNIQUE IA — PLATEFORMES LLM ON-PREMISE, GPU, KUBERNETES ET LLMOPS
...
Travaille la plateforme comme un service rendu à des équipes GenAI : mise à disposition des modèles, maîtrise de la latence et du coût par requête, routage entre modèles, mise en cache, supervision et dimensionnement du parc. Documentation d'exploitation, modularité et réversibilité systématiques : les plateformes livrées sont conçues pour être tenues par les équipes internes.
...
### Plateforme de service des modèles et exploitation GPU

- Mise à disposition de modèles en interne sous forme de service, sélection du moteur de service selon la charge et les contraintes
- Batching continu et gestion du cache d'attention pour l'occupation réelle du parc, quantification pour réduire l'empreinte mémoire
- Routage entre modèles selon la complexité de la tâche, mise en cache sémantique, repli, maîtrise du coût par requête
- Dimensionnement à partir de la mesure : profil de charge, latence p50, p95 et p99, tokens par cas d'usage, taux d'occupation
- Priorisation des files et dégradation gracieuse entre usages d'exploration et usages d'exploitation
- Conteneurisation Docker et orchestration Kubernetes, déploiement en environnement on-premise maîtrisé
- Intégration et déploiement continus pour les modèles, les images et les prompts, GitLab CI et GitHub Actions
- Supervision et observabilité : Prometheus, Grafana, OpenTelemetry, Langfuse, MLflow, indicateurs de latence, d'erreur et de consommation
- Documentation d'exploitation et transfert aux équipes internes
...
- Infrastructure : Linux, Docker, Kubernetes, GPU, Terraform, GitLab CI/CD, GitHub Actions, Bash, Python

% HAKIM AREZKI, PhD
% ARCHITECTE ET TECH LEAD IA — AGENTS, MCP, INDUSTRIALISATION ET GOUVERNANCE DES COMPOSANTS
...
Traite les composants d'IA comme un patrimoine à gouverner, pas comme des livrables isolés : inventaire et qualification des agents et des outils existants, définition des critères de publication, chaînes d'intégration continue avec contrôles de qualité et de sécurité, revues de code, documentation et mise à disposition auprès des équipes techniques et métiers. Les solutions livrées sont modulaires, réversibles et conçues pour être reprises en exploitation par les équipes internes.
...
### Catalogue et industrialisation des composants agentiques

- Inventaire et qualification des agents, des outils et des serveurs exposés, critères d'entrée et niveaux de maturité
- Model Context Protocol, conception de serveurs exposant données métiers et API internes, schémas d'entrée et de sortie contractualisés
- Contrats d'interface décrits en JSON Schema, sorties structurées, validation et compatibilité ascendante
- Processus de publication et d'onboarding : documentation minimale exigée, propriétaire identifié, cycle de vie et dépréciation
- Chaînes d'intégration continue avec contrôles de qualité, analyse statique et contrôles de sécurité
- Revue de code, standards de développement, tests et bornage des exécutions
- Observabilité des composants en service : traces, coût, latence, taux d'échec
- Mise à disposition auprès des équipes techniques et métiers, guides d'usage et exemples exécutables
...
- Frameworks et protocoles : LangChain, LlamaIndex, Semantic Kernel, MCP (Model Context Protocol), JSON Schema, MLflow, PyTorch, TensorFlow, scikit-learn
...
- Industrialisation : Linux, Docker, Kubernetes, Terraform, GitLab CI/CD, GitHub Actions, analyse statique et contrôles de sécurité dans la chaîne de build
